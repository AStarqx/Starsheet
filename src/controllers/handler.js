...
export default function luckysheetHandler() {
    ...
    $("#luckysheet-cell-main, #luckysheetTableContent")
            ...
            if (parseInt($input.css("top")) > 0) {
                if (
                    ...
                    setTimeout(function() {
                        ...
                        if ($span && $span != undefined && $span != null && $span && $span.html() && $span.html().length) {
        ...
        .mouseup(function(event) {
            if (event.which == "3") {
                ...
                if (obj_s["row"] != null && obj_s["row"][0] == 0 && obj_s["row"][1] == Store.flowdata.length - 1) {
                    ...
                    if (isSame) {
                        const size = parseFloat(first_collen / 96 * 2.54).toFixed(1);
                    ...
                    if (isSame) {
                        const size = parseFloat(first_rowlen / 96 * 2.54).toFixed(1);
                        $("#luckysheet-cols-rows-add")
                            .find("input[type='number'].rcsize")
                            .val(size);
                    } else {
        ...
        .dblclick(function(event) {
            ...
            if (Store.flowdata[row_index] != null && Store.flowdata[row_index][col_index] != null) {
                ...
                // 检测是否为链接
                if(/^http[s]?:\/\//.test(cell.v)) {
                    const _locale = locale();
                    const hyperlinkText = _locale.insertLink;
                    hyperlinkCtrl.addHyperLink(row_index, col_index, {
                        linkText: cell.v,
                        linkType: 'external',
                        linkAddress: cell.v,
    ...
    $(document).on("mousemove.luckysheetEvent", function(event) {
        ...
        if (luckysheetConfigsetting && luckysheetConfigsetting.hook && luckysheetConfigsetting.hook.sheetMousemove) {
            ...
            let tableContent = $("#luckysheetTableContent").get(0);
            let luckysheetTableContent = tableContent ? tableContent.getContext("2d") : null;

            if (Store.flowdata && Store.flowdata[row_index] && Store.flowdata[row_index][col_index]) {
                method.createHookFunction(
                    "sheetMousemove",
                    Store.flowdata[row_index][col_index],
                    {
    ...
    $("#luckysheetChangeImage").click(function() {
        if (!checkProtectionAuthorityNormal(Store.currentSheetIndex, "editObjects")) {
            return;
        }
        imgType = 2;
    ...
    $("#luckysheet-imgUpload").on("change", function(e) {
        ...
        imageCtrl.insertImg(file, imgType);
    });
    ...
    $("#luckysheet-importXlsx-btn-title").click(function() {
        ...
        $("#luckysheet-xlsxUpload").click();
    });
    $("#luckysheet-xlsxUpload").click(function(e) {
        e.stopPropagation();
    });
    $("#luckysheet-xlsxUpload").on("change", function(e) {
        if (!checkProtectionAuthorityNormal(Store.currentSheetIndex, "editObjects", false)) {
            return;
        }
        let file = e.currentTarget.files[0];
    ...
    $("#luckysheet-exportXlsx-btn-title").click(function() {
        ...
        // }
        let sheets = Store.luckysheetfile
        xlsxCtrl.exportExcel(sheets)
    });
        ...
        if (selection.isPasteAction) {
            ...
            if (
                ...
                //复制的是图片
                    ...
                    imageCtrl.insertImg(clipboardData.files[0], Store.currentImgId ? 2 : 1);
