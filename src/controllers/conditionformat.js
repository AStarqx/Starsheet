...
import { setRangeConditionalFormatDefault } from '../global/api'
...
const conditionformat = {
    ...
    init: function(){
        ...
        // 定义字段
        $(document).off("click.CFdefinedFieldsConfirm").on("click.CFdefinedFieldsConfirm", "#luckysheet-definedFields-dialog-confirm", function(){
            let sheets = $.extend(true, [], Store.luckysheetfile);
            let currSheet = sheets[getSheetIndex(Store.currentSheetIndex)];
            let rangeNames = currSheet.rangeNames || [];
            const rangeName = $("#rangeName").val().trim();
            if(!rangeName){
            ...
            if(conditionformat.definedFieldType !== 'default') {
                ...
                let startRow = parseFloat(cell[0].replace(/[^\d]/g, '')) - 1;
                let endColumn = ABCatNum(cell[1].replace(/[^a-zA-z]/g, ''));
                let endRow = parseFloat(cell[1].replace(/[^\d]/g, '')) - 1;
                if($("#isNeedQuote").is(':checked')){
                    setRangeConditionalFormatDefault('rangeName', [1], {
                        cellrange: [ 
                            { row: [startRow, endRow], column: [startColumn, endColumn] }
                        ],
                        cellRangeName: rangeName + '_评估方法引用',
                        success: (e) => {
                            rangeNames.push({ name: rangeName + '_评估方法引用', order: getSheetIndex(Store.currentSheetIndex), range: range })
                ...
                setRangeConditionalFormatDefault('rangeName', [1], {
                    ...
                    cellRangeName: rangeName,
                    cellImageId: Store.currentImgId,
                    success: (e) => {
                        rangeNames.push({ name: rangeName, order: getSheetIndex(Store.currentSheetIndex), range: range, cellImageId: Store.currentImgId })
                        $("#luckysheet-modal-dialog-mask").hide();
                        $("#luckysheet-definedFields-dialog").hide();
                    }
                })
        ...
        $(document).off("click.CFadministerRuleConfirm").on("click.CFadministerRuleConfirm", "#luckysheet-administerRule-dialog-confirm", function(){
            ...
            currentRules.forEach(rule => {
                if(rule.luckysheet_conditionformat_save) {
                    // 获取所有定义字段
                    const allFields = rule.luckysheet_conditionformat_save.filter(f => f && f.conditionName && f.conditionName === 'rangeName')
                    allFields.forEach(f => {
                        const range = f.cellrange
                        const startColumnNo = chatatABC(range[0].column[0]);
                        const startRowNo = range[0].row[0] + 1;
                        const startCellNo = `${startColumnNo}${startRowNo}`;
                        const endColumnNo = chatatABC(range[0].column[1]);
                        const endRowNo = range[0].row[1] + 1;
                        const endCellNo = `${endColumnNo}${endRowNo}`;
                        const cellRange = `${startCellNo}:${endCellNo}`;

                        const currRangeNames = Store.luckysheetfile[getSheetIndex(rule.sheetIndex)]["rangeNames"]
                        const currRangeName = currRangeNames.find(r => r.name === f.cellRangeName)
                        if(currRangeName) {
                            currRangeName.range = cellRange
                        }
                    })
                }
            })
        ...
        $(document).on("click", ".range .fa-table", function(){
            ...
            else if(id == "luckysheet-definedFields-dialog") {
                let $id = $(this).parents(".range").attr("id");
                if($id == "formulaConditionVal"){
                    source = "3_0";
                }
        ...
        $(document).on("click", "#luckysheet-singleRange-dialog-confirm", function(){
            ...
            if(source == "0_1"){
                ...
                $("#luckysheet-definedFields-dialog").show();
                $("#luckysheet-definedFields-dialog #conditionVal input").val(v);
            }
            else if(source == "3_2") {
                $("#luckysheet-definedFields-dialog").show();
                $("#luckysheet-definedFields-dialog #conditionVal2 input").val(v);
            }
        ...
        $(document).on("click", "#luckysheet-singleRange-dialog-close", function(){
            ...
            else if(source == "3_0" || source == "3_1" || source == "3_2"){
                $("#luckysheet-definedFields-dialog").show();
            }
    ...
    // 定义字段
    definedFieldsDialog: function(){
        $("#luckysheet-modal-dialog-mask").show();
        $("#luckysheet-definedFields-dialog").remove();
        const conditionformat_Text = locale().conditionformat;

        const range = $.extend(true, [], Store.luckysheet_select_save);
        const startColumnNo = chatatABC(range[0].column[0]);
        const startRowNo = range[0].row[0] + 1;
        const startCellNo = `${startColumnNo}${startRowNo}`;
        const endColumnNo = chatatABC(range[0].column[1]);
        ...
        // <div class="multi" id="conditionVal">
        ...
        ` : `
            <div class="box-item">
                <span class="item-label">引用位置：</span>
                <input id="range" class="item-control" value="${cellRange}" readonly="true"></input>
            </div>
            <div class="box-item">
                <span class="item-label"></span>
                <input type="checkbox" id="isNeedQuote"></input>
        ...
        let content =  `
                        <div class="definedFieldsBox">
                            <div class="box-item">
                                <span class="item-label">字段名称：</span>
                                <input id="rangeName" class="item-control" autofocus></input>
                            </div>
                            ${dynamicHtml}
                        </div>`;
        ...
        let $t = $("#luckysheet-definedFields-dialog")
                .find(".luckysheet-modal-dialog-content")
                .css("min-width", 400)
                .end(),
            myh = $t.outerHeight(),
        ...
        let winw = $(window).width(), winh = $(window).height();
        let scrollLeft = $(document).scrollLeft(), scrollTop = $(document).scrollTop();
        $("#luckysheet-definedFields-dialog").css({
            "left": (winw + scrollLeft - myw) / 2,
            "top": (winh + scrollTop - myh) / 3
        }).show();
        $("#rangeName").focus();
    },
    administerRuleDialog: function(bool){
        let _this = this;
        _this.isRangeNameModel = bool;
        $("#luckysheet-modal-dialog-mask").show();
        ...
        if(bool){
            content =  `<div class="chooseSheet">
                        ...
                        <div class="ruleBox">
                            ...
                            <div class="ruleList">
                                <div class="listTitle">
                                    <span>${conditionformat_Text.rangeName}</span>
                                    <span>${conditionformat_Text.applyNameRange}</span>
                                </div>
                                <div class="listBox"></div>
                            </div>
                        </div>`;
        ...
        //当前工作表的规则列表
        let index = $("#luckysheet-administerRule-dialog .chooseSheet option:selected").val();
        this.getConditionRuleList(index, bool);
    },
    getConditionRuleList: function(index, bool){
        ...
        if(ruleArr != null && ruleArr.length > 0){
            ...
            for(let i = 0; i < ruleArr.length; i++){
                ...
                let itemHtml =  '<div class="item" data-item="' + i + '">' +
                                ...
                                '</div>';
                // 选区名称dom
                if(ruleArr[i].conditionName == "rangeName"){
                    itemHtml = '';
                }
                if(bool){
                    if(ruleArr[i].conditionName == "rangeName") {
                        let rangeText = ''
                        if(ruleArr[i].cellImageId) rangeText = ruleArr[i].cellImageId
                        if(rangeTxtArr && rangeTxtArr.length) {
    ...
    getConditionRuleName: function(conditionName, conditionRange, conditionValue){
        ...
        else if(conditionName == "rangeName"){
            return conditionformat_Text.ruleRangeName;
        }
    },
