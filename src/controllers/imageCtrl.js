...
const imageCtrl = {
    ...
    cropChangeObj: null,
    copyImgItemObj: null,
    insertImg: function (file, imgType) {
        let _this = this
        imgType = imgType || 1;
        ...
        if (typeof uploadImage === 'function') {
            ...
            uploadImage(file).then(url => {
                imageCtrl._insertImg(url, imgType);
            }).catch(error => {
            ...
            render.onload = function(event){
                ...
                imageCtrl._insertImg(src, imgType);
                $("#luckysheet-imgUpload").val("");
            }
        }
    },

    _insertImg: function(src, imgType){
        let _this = this;
    ...
    modelHtml: function(id, imgItem) {
        let _this = this;
        let defaultZoomRatio = imgItem.zoomRatio ? imgItem.zoomRatio : 1
        let StoreZoomRatio = Store.zoomRatio ? Store.zoomRatio : 1
        let zoomRatio = StoreZoomRatio / defaultZoomRatio
        let imageUrlHandle = Store.toJsonOptions && Store.toJsonOptions['imageUrlHandle'];
        ...
        return  `<div id="${id}" class="luckysheet-modal-dialog luckysheet-modal-dialog-image" style="width:${width}px;height:${height}px;padding:0;position:${position};left:${left}px;top:${top}px;z-index:200;">
                    <div class="luckysheet-modal-dialog-content" style="width:100%;height:100%;overflow:hidden;position:relative;">
                        <img src="${src}" style="position:absolute;width:${imgItem.default.width * zoomRatio}px;height:${imgItem.default.height * zoomRatio}px;left:${-imgItem.crop.offsetLeft * zoomRatio}px;top:${-imgItem.crop.offsetTop * zoomRatio}px;" />
                    </div>
                    <div class="luckysheet-modal-dialog-border" style="border:${borderWidth}px ${imgItem.border.style} ${imgItem.border.color};border-radius:${imgItem.border.radius * zoomRatio}px;position:absolute;left:${-borderWidth}px;right:${-borderWidth}px;top:${-borderWidth}px;bottom:${-borderWidth}px;"></div>
                </div>`;
    },
    ...
    sliderHtmlShow: function() {
        ...
        Store.currentImgId = _this.currentImgId;
    ...
    init: function() {
        ...
        $("#luckysheet-image-showBoxs").off("mousedown.active").on("mousedown.active", ".luckysheet-modal-dialog-image", function(e) {
            ...
            let top = imgItemParam.top;
            let position = imgItemParam.position;
            let defaultZoomRatio = item.zoomRatio ? item.zoomRatio : 1
            let StoreZoomRatio = Store.zoomRatio ? Store.zoomRatio : 1
            let zoomRatio = StoreZoomRatio / defaultZoomRatio
            ...
            $("#luckysheet-modal-dialog-activeImage .luckysheet-modal-dialog-content").css({
                ...
                "background-size": item.default.width * zoomRatio + "px " + item.default.height * zoomRatio + "px",
                "background-position": -item.crop.offsetLeft * zoomRatio + "px " + -item.crop.offsetTop * zoomRatio + "px"
            })

            $("#luckysheet-modal-dialog-activeImage .luckysheet-modal-dialog-border").css({
                "border-width": item.border.width * zoomRatio,
                "border-style": item.border.style,
                "border-color": item.border.color,
                "border-radius": item.border.radius * zoomRatio,
                "left": -item.border.width * zoomRatio,
                "right": -item.border.width * zoomRatio,
                "top": -item.border.width * zoomRatio,
                "bottom": -item.border.width * zoomRatio,
            })
    ...
    getImgItemParam(imgItem){
        ...
        let StoreZoomRatio = Store.zoomRatio ? Store.zoomRatio : 1
        let zoomRatio = StoreZoomRatio / defaultZoomRatio
        let width = imgItem.default.width * zoomRatio,
            height = imgItem.default.height * zoomRatio,
            left = imgItem.default.left * zoomRatio,
            top = imgItem.default.top * zoomRatio;
        if(imgItem.crop.width != width || imgItem.crop.height != height){
            width = imgItem.crop.width * zoomRatio;
            height = imgItem.crop.height * zoomRatio;
            left += imgItem.crop.offsetLeft * zoomRatio;
            top += imgItem.crop.offsetTop * zoomRatio;
        }
        ...
        if(isFixedPos){
            ...
            left = (left - operateAreaWidth) - (left - operateAreaWidth) * zoomRatio + operateAreaWidth
            top = (top - operateAreaHeight) - (top - operateAreaHeight) * zoomRatio + operateAreaHeight
        }
    ...
    cancelActiveImgItem: function(){
        ...
        $("#" + _this.currentImgId + " .luckysheet-modal-dialog-border").css({
            "border-width": imgItem.border.width * zoomRatio,
            "border-style": imgItem.border.style,
            "border-color": imgItem.border.color,
            "border-radius": imgItem.border.radius * zoomRatio,
            "left": -imgItem.border.width * zoomRatio,
            "right": -imgItem.border.width * zoomRatio,
            "top": -imgItem.border.width * zoomRatio,
            "bottom": -imgItem.border.width * zoomRatio,
        })

        _this.currentImgId = null;
        Store.currentImgId = null;
    },
    addImgItem: function(img, imgType) {
        let _this = this;
        ...
        let left = img.left
        let top = img.top
        let originWidth = img.originWidth
        let originHeight = img.originHeight
        if(imgType === 2) {
            let oldImgItem = _this.images[_this.currentImgId];
            let oldImgItemParam = _this.getImgItemParam(oldImgItem)
            left = oldImgItemParam.left
            top = oldImgItemParam.top
            width = oldImgItemParam.width
            height = oldImgItemParam.height
        }
        imgItem.src = img.src;
        imgItem.originWidth = originWidth;
        imgItem.originHeight = originHeight;
        imgItem.default.width = width;
        imgItem.default.height = height;
        imgItem.default.left = left;
        imgItem.default.top = top;
        imgItem.crop.width = width;
        imgItem.crop.height = height;
        imgItem.zoomRatio = Store.zoomRatio

        let scrollTop = $("#luckysheet-cell-main").scrollTop(), 
            scrollLeft = $("#luckysheet-cell-main").scrollLeft();

        imgItem.fixedLeft = left - scrollLeft + Store.rowHeaderWidth;
        imgItem.fixedTop = top - scrollTop + Store.infobarHeight + Store.toolbarHeight + Store.calculatebarHeight + Store.columnHeaderHeight;
        let id = imgType === 1 ? _this.generateRandomId() : Store.currentImgId;
        ...
        if(imgType === 2) {
            $(`#${Store.currentImgId}`).remove();
            let modelHtml = _this.modelHtml(Store.currentImgId, imgItem);
            $("#luckysheet-image-showBoxs .img-list").append(modelHtml);
            setTimeout(() => {
                _this.cancelActiveImgItem();
            }, 30);
    ...
    removeImgItem: function() {
        ...
        const conditionformat = currSheet.luckysheet_conditionformat_save || [];
        let allImageRange = conditionformat.filter(f => f.cellImageId);
        if(allImageRange.find(r => r.cellImageId === _this.currentImgId)) {
            return tooltip.info('此图片已定义字段,无法删除.', '');
        }
        let imgItem = _this.images[_this.currentImgId];
