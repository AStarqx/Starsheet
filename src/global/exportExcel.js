...
export async function getSheetExcel(luckysheet,name="file"){
    // 1.创建工作簿，可以为工作簿添加属性
    const workbook = new Excel.Workbook();
    // 2.创建表格，第二个参数可以配置创建什么样的工作表
    luckysheet.every(function (table) {
...
var getWorksheetOptions = function(table){
    let tabColor = !table.color?'':(table.color+"").indexOf('rgb')>-1?rgb2hex(table.color):table.color;
    let showGridLines = table.showGridLines && table.showGridLines == '1' ? false : true;
    let printArea = table.printArea ? table.printArea.join('&&') : '';
    let options = {
        properties: {
            tabColor: {
                argb: tabColor.replace('#', '')
            }
        },
        pageSetup: {
            paperSize: 9,
        ...
        views: [
            { 
                // showGridLines: showGridLines  // 是否显示网格
...
var setMerge = function (table, worksheet) {
  if(!table.config) return
  const luckyMerge = (table.config && table.config.merge) || {}
...
var setImages = function (table, worksheet, workbook) {
    ...
    if(!visibledatacolumn){
        visibledatacolumn = [];
        let prevWidth = 0;
        for (const key in table.config.columnlen) {
            let column = table.config.columnlen[key];
            let width = prevWidth + column;
            visibledatacolumn.push(width);
            prevWidth = width;
        }
    }
    if(!visibledatarow){
        visibledatarow = [];
        let prevHeight = 0;
        for (const key in table.config.rowlen) {
            let row = table.config.rowlen[key];
            let height = prevHeight + row;
            visibledatarow.push(height);
            prevHeight = height;
        }
    }
    ...
    // 通过 base64  将图像添加到工作簿
        ...
        worksheet.addImage(imageId, {
            tl: { col: col_st, row: Math.round(row_st)},
...
var getBorderInfo = function(luckysheetfile){
  ...
  let borderInfo = cfg ? cfg["borderInfo"] : [];
  //设置需要计算边框的区域
...
var getRowHeight = function(rowInfo, cfg, options = {}) {
    if(getObjType(rowInfo) != 'array' || rowInfo.length == 0){
        return tooltip.info("The rowInfo parameter is invalid.", "");
    }
    let rowlen = cfg ? cfg["rowlen"] : undefined;
    let rowlenObj = {};
    rowInfo.forEach((item) => {
        if(parseInt(item) >= 0){
            let size = rowlen ? rowlen[parseInt(item)] : 19;
...
var getColumnWidth = function(columnInfo, cfg, options = {}) {
    ...
    columnInfo.forEach((item) => {
        if(parseInt(item) >= 0){
            let size = columnlen[parseInt(item)] || 73;
...
var getColumnHidden = function (columnInfo, cfg) {
    if(getObjType(columnInfo) != 'array' || columnInfo.length == 0){
        return tooltip.info("The columnInfo parameter is invalid.", "");
    }
    let colhidden = cfg["colhidden"] || {};
    let hidden = 1;
    columnInfo.forEach((item) => {
        if(parseInt(item) >= 0){
            hidden = colhidden[parseInt(item)];
        }
    })
    return hidden === 0;
}
...
var setStyleAndValue = function (table, worksheet) {
    ...
    if(table.frozen && table.frozen.range) {
        ...
        let ySplit = table.frozen.type == 'rangeColumn' ? 0 : table.frozen.range.row_focus + 1
        worksheet.views = [
            { state: 'frozen', xSplit: xSplit, ySplit: ySplit }
        ]
    } 
