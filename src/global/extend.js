...
function luckysheetextendtable(type, index, value, direction, sheetIndex) {
    ...
    //打印区域变动
    let printarea_select = file.printarea_select;
    let newPrintareaObj = null;
    if(printarea_select != null && JSON.stringify(printarea_select) != "{}"){
        newPrintareaObj = { "printarea_select": null };
    ...
    if (file.index == Store.currentSheetIndex) {
        ...
        file.filter = newFilterObj ? newFilterObj.filter : {};
        file.filter_select = newFilterObj ? newFilterObj.filter_select : {};
...
function luckysheetdeletetable(type, st, ed, sheetIndex) {
    ...
    if (file.index == Store.currentSheetIndex) {
        ...
        file.filter = newFilterObj ? newFilterObj.filter : {};
        file.filter_select = newFilterObj ? newFilterObj.filter_select : {};
        file.printarea_select = newPrintareaObj.printarea_select;
        file.luckysheet_conditionformat_save = newCFarr;
        file.luckysheet_alternateformat_save = newAFarr;
        file.dataVerification = newDataVerification;
        file.hyperlink = newHyperlink;
...
function luckysheetDeleteCell(type, str, edr, stc, edc, sheetIndex) {
    ...
    if(printarea_select != null && JSON.stringify(printarea_select) != "{}"){
        ...
        if(type == 'moveUp'){
            if(f_c1 >= stc && f_c2 <= edc){
                if(f_r1 > edr){
                    newPrintareaObj.printarea_select = {
                        "row": [f_r1 - rlen, f_r2 - rlen],
                        "column": [f_c1, f_c2]
                    }
                }
