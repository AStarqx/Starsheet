...
function getMeasureText(value, ctx, fontset){
    ...
    if(mtc != null){
        ...
        cache.fontBoundingBoxDescent = measureText.fontBoundingBoxDescent;
        cache.fontBoundingBoxAscent = measureText.fontBoundingBoxAscent;
        ...
        if(cache.fontBoundingBoxDescent==null || cache.fontBoundingBoxAscent==null || isNaN(cache.fontBoundingBoxDescent) || isNaN(cache.fontBoundingBoxAscent)){
            let commonWord = "M"
            if(hasChinaword(value)){
                commonWord = "田";
            }
            let oneLineTextHeight = menuButton.getTextSize(commonWord, ctx.font)[1]*0.8;
            if(ctx.textBaseline=="top"){
                cache.fontBoundingBoxDescent = oneLineTextHeight;
                cache.fontBoundingBoxAscent = 0;
            }
            else if(ctx.textBaseline=="middle"){
                cache.fontBoundingBoxDescent = oneLineTextHeight/2;
                cache.fontBoundingBoxAscent = oneLineTextHeight/2;
            }
            else{
                cache.fontBoundingBoxDescent = 0;
                cache.fontBoundingBoxAscent = oneLineTextHeight;
            }
        ...
        if(ctx.textBaseline == 'alphabetic'){
            ...
            if(cache.fontBoundingBoxDescent<=matchTextMeasure.fontBoundingBoxDescent){
                cache.fontBoundingBoxDescent = descTextMeasure.fontBoundingBoxDescent;
                if(cache.fontBoundingBoxDescent==null){
                    cache.fontBoundingBoxDescent = 0;
                }
            }
        ...
        cache.fontBoundingBoxDescent *= Store.zoomRatio;
        cache.fontBoundingBoxAscent *= Store.zoomRatio;
...
function getCellTextInfo(cell , ctx, option){
    ...
    if(tr=="3"){//vertical text
        ...
        if(tb=="2" || isInline){//wrap
            ...
            let textWidth = measureText.width;
            let textHeight = measureText.actualBoundingBoxDescent + measureText.actualBoundingBoxAscent;
            // let textHeight = measureText.fontBoundingBoxDescent + measureText.fontBoundingBoxAscent;
            textContent.rotate = rt;
            rt = Math.abs(rt);
            let rtPI = rt*Math.PI/180;
