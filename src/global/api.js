 ...
 * @param {Function} options.success 操作结束的回调函数, callback参数为改变后的cell对象
 */
export function setCellFormatV2(row, column, attr, value, options = {}) {
    if (!isRealNum(row) || !isRealNum(column)) {
        return tooltip.info('Arguments row or column cannot be null or undefined.', '')
    }
    ...
    if (attr == 'bd') {
        ...
        // cellData[attr] = value;
        menuButton.updateFormatV2(targetSheetData, attr, value, row, column)
    }
...
export function setRangeConditionalFormatDefault(conditionName, conditionValue, options = {}) {
    ...
    let rule = {
        "type": "default",
        "cellrange": cellrange,
        "cellRangeName": cellRangeName,
        'cellImageId': cellImageId,
        "format": format,
 ...
 * 设置sheet数据
 * @param {Object} options 可选参数
 * @param {Object} options.sheetObject 工作表的数据；默认值为空对象
 * @param {Number} options.order 工作表索引；默认值为最后一个索引位置
 * @param {Function} options.success 操作结束的回调函数
 */
export function setSheetData(options = {}) {
    ...
    let index = sheetObject.index
    let sheetname = sheetObject.name
    ...
    if(order <= 0){
        ...
        Store.luckysheetfile.splice(0, 1, sheetconfig);
    }
    else{
        ...
        Store.luckysheetfile.splice(order, 1, sheetconfig);
    }
    ...
    Store.flowdata = sheetconfig.data
    Store.config = sheetconfig.config
    ...
    setTimeout(() => {
        ...
        if(index == Store.currentSheetIndex){
            if("rowhidden" in Store.config || "colhidden" in Store.config || "rowlen" in Store.config || "columnlen" in Store.config){
                jfrefreshgrid_rhcw(Store.flowdata.length, Store.flowdata[0].length);
            }
...
export function setSheetAdd(options = {}) {
    ...
    if(sheetObject && sheetObject.name) {
        let txt = sheetObject.name;
